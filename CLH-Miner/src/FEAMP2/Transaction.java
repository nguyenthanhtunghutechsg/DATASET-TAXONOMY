...
public class Transaction {
	...
	public HashSet<Integer> parentsInTransaction = new HashSet<Integer>();
	public HashSet<Integer> UnpromisingsInTransaction = new HashSet<Integer>();
	...
	public Transaction(Transaction transaction, int itemE, int[] newNamesToOldNames, TaxonomyTree taxonomy,
		...
		this.parentsInTransaction = new HashSet<Integer>();
		this.UnpromisingsInTransaction = new HashSet<Integer>();
		for (Integer parent : transaction.parentsInTransaction) {
			boolean i = (!CheckParent(parent, newNamesToOldNames[itemE], taxonomy));
			if (i && itemE < oldNamesToNewNames[parent]) {
				parentsInTransaction.add(parent);
			}
		}
		for (Integer unpromising : transaction.UnpromisingsInTransaction) {
			UnpromisingsInTransaction.add(unpromising);
		}

	}

	public Transaction(Transaction transaction, int ParentItem, TaxonomyTree taxonomy, int[] newNamesToOldNames,
			HashMap<Integer, Map<Integer, Double>> mapItemToParentInTransaction,int[] oldNamesToNewNames) {
		this.Tid = transaction.Tid;
		this.OldTU = transaction.OldTU;
		double newTU = transaction.getUtility();
		int newSize = 0;
		...
		this.utilities = new double[newSize];
		System.arraycopy(tempUtilities, 0, this.utilities, 0, newSize);
		double utilityofParent = mapItemToParentInTransaction.get(transaction.Tid).get(newNamesToOldNames[ParentItem]);
		...
		for (Integer parent : transaction.parentsInTransaction) {
			boolean i = (!CheckParent(parent, newNamesToOldNames[ParentItem], taxonomy));
			if (i && ParentItem < oldNamesToNewNames[parent]) {
				parentsInTransaction.add(parent);
			}
		}
		for (Integer unpromising : transaction.UnpromisingsInTransaction) {
			if (!CheckParent(unpromising, newNamesToOldNames[ParentItem], taxonomy)) {
				UnpromisingsInTransaction.add(unpromising);
			}
	...
	public void removeUnpromisingItems(int[] oldNamesToNewNames, TaxonomyTree taxonomy,
			HashMap<Integer, Map<Integer, Double>> mapItemToParentInTransaction,
			HashMap<Integer, Map<Integer, Double>> mapItemToUnPromissing) {
		...
		Map<Integer, Double> mapParentToUtility = new HashMap<Integer, Double>();
		Map<Integer, Double> mapUnpromissingToUtility = new HashMap<Integer, Double>();
		for (int j = 0; j < items.length; j++) {
			...
			while (nodeParent.getData() != -1) {
				int itemParent = nodeParent.getData();
				if (oldNamesToNewNames[itemParent] != 0) {
					Double UtitilityOfParent = mapParentToUtility.get(itemParent);
					if (UtitilityOfParent == null) {
						mapParentToUtility.put(itemParent, utility);
					} else {
						mapParentToUtility.put(itemParent, UtitilityOfParent + utility);
					}
					parentsInTransaction.add(itemParent);
				}
			...
			if (newName != 0) {
				...
				UnpromisingsInTransaction.add(item);
				mapUnpromissingToUtility.put(item, utility);
		...
		mapItemToParentInTransaction.put(Tid, mapParentToUtility);
		mapItemToUnPromissing.put(Tid, mapUnpromissingToUtility);
