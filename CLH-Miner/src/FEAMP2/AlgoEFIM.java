...
public class AlgoEFIM {
	...
	public HashMap<Integer, Map<Integer, Double>> mapItemToParentInTransaction;
	public HashMap<Integer, Map<Integer, Double>> mapItemToUnPromissing;
	...
	public void runAlgorithm(int minUtil, String inputPath, String outputPath, String TaxonomyPath,
		...
		mapItemToParentInTransaction = new HashMap<Integer, Map<Integer, Double>>();
		mapItemToUnPromissing = new HashMap<Integer, Map<Integer, Double>>();
		// read the input file
		...
		for (int i = 0; i < dataset.getTransactions().size(); i++) {
			...
			transaction.removeUnpromisingItems(oldNameToNewNames, taxonomy, mapItemToParentInTransaction,
					mapItemToUnPromissing);
		}
		...
		for (int x = 0; x < itemsToExplore.size(); x++) {
			...
			if (taxonomy.getMapItemToTaxonomyNode().get(newNamesToOldNames[e]).getChildren().size() == 0) {
				for (Transaction transaction : transactionsOfP) {
					...
					if (positionE > -1) {
						Transaction projectedTransaction = new Transaction(transaction, e, newNamesToOldNames, taxonomy,
								oldNameToNewNames);
				...
				for (Transaction transaction : transactionsOfP) {
					// Increase the number of transaction read
					transactionReadingCount++;
					if (transaction.parentsInTransaction.contains(newNamesToOldNames[e])) {
						Transaction projectedTransaction = new Transaction(transaction, e, taxonomy,
								newNamesToOldNames,mapItemToParentInTransaction,oldNameToNewNames);
						utilityPe += projectedTransaction.prefixUtility;
						transactionsPe.add(projectedTransaction);
					}
				}
	...
	public void useUtilityBinArrayToCalculateSubtreeUtilityFirstTime(Dataset dataset) {
		...
		for (Transaction transaction : dataset.getTransactions()) {
			...
			sumSU = 0;
			for (int itemParent : transaction.parentsInTransaction) {
				sumSU = 0;
				...
				for (int i = 0; i < transaction.items.length; i++) {
					...
					if (CheckParent(itemParent, newNamesToOldNames[item]) == false
							&& oldNameToNewNames[itemParent] < item) {
						sumSU += transaction.getUtilities()[i];
					}
				}
				for (int itemUnpromising : transaction.UnpromisingsInTransaction) {
					if (!CheckParent(itemParent, itemUnpromising)) {
						sumSU += mapItemToUnPromissing.get(transaction.Tid).get(itemUnpromising);
					}
				}
				sumSU += mapItemToParentInTransaction.get(transaction.Tid).get(itemParent);
				utilityBinArraySU[oldNameToNewNames[itemParent]] += sumSU;
			}
			sumSU = 0;
			for (int itemUnpromising : transaction.UnpromisingsInTransaction) {
				sumSU += mapItemToUnPromissing.get(transaction.Tid).get(itemUnpromising);
	...
	private void useUtilityBinArraysToCalculateUpperBounds(List<Transaction> transactionsPe, int j,
		...
		for (Transaction transaction : transactionsPe) {
			transactionReadingCount++;
			double countUnpromise = 0;
			for (Integer IntemUnpromising : transaction.UnpromisingsInTransaction) {
				countUnpromise+=mapItemToUnPromissing.get(transaction.Tid).get(IntemUnpromising);
			}
			sumRemainingUtility = countUnpromise; 
			for (int i = transaction.getItems().length - 1; i >=0 ; i--) {
				// get the item
			...
			for (Integer itemParent : transaction.parentsInTransaction) {
				double sumU = transaction.OldTU;
				double SumLU = transaction.OldTU;
				// We add the utility of this item to the sum of remaining utility
				for (int i = 0; i < transaction.getItems().length; i++) {
					int item = transaction.getItems()[i];
					if (CheckParent(itemParent, newNamesToOldNames[item]) == false) {
						if (oldNameToNewNames[itemParent] > item) {
				...
				utilityBinArraySU[oldNameToNewNames[itemParent]] += sumU;
				utilityBinArrayLU[oldNameToNewNames[itemParent]] += transaction.prefixUtility+transaction.transactionUtility;
